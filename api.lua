...
--			print(" ** stop attacking **", dist, self.view_range)
		...
		if self.attack_type == "explode" then
			...
			if not self.v_start and dist <= self.reach and line_of_sight(self, s, p, 2) then
				self.v_start = true
				self.timer = 0
				self.blinktimer = 0
...
--				print ("=== explosion timer started", self.explosion_timer)
			...
			elseif self.allow_fuse_reset and self.v_start and (dist > math.max(self.reach, entity_damage_radius) + 0.5 or not line_of_sight(self, s, p, 2)) then
				self.v_start = false
				self.timer = 0
				self.blinktimer = 0
